> **Documento histórico.** Relevamiento de los 25 bugs del MVP, **todos cerrados** entre la
> 0.3.0 y la 0.4.0. Se conserva por el detalle forense —causa, solución y evidencia de cada
> uno—, que explica por qué varias partes del código son como son, y porque el
> [`CHANGELOG`](../../CHANGELOG.md) lo referencia.
>
> La sección *Trabajo transversal* del final quedó desactualizada: los tests existen, los
> listados paginan y el logging está. Lo que sigue vivo de ahí está en
...
1. **Prohibir anular una venta ya cobrada.** Si está cobrada, exigir un flujo explícito de
   devolución (fuera de alcance del MVP; anotarlo en `roadmap.md`).
